...
def build_dataloader(cfg, dataset):
    if cfg.distributed:
        if dataset.which_set == "train":
            ...
            sampler = DistributedSampler(dataset, cfg.world_size, cfg.rank, shuffle=False, seed=cfg.seed)
    else:
        sampler = GroupSampler(dataset, cfg.data.samples_per_gpu) if dataset.which_set == "train" else None
    #    sampler = RandomSampler(dataset) if dataset.which_set == "train" else None

    init_fn = (
        partial(worker_init_fn, num_workers=cfg.data.workers_per_gpu, rank=cfg.rank, seed=cfg.seed)
