...
class DefaultFormatBundle:
    ...
    def __call__(self, results):
        ...
        if "img_mini" in results:
            img = results["img_mini"]
            if len(img.shape) < 3:
                img = numpy.expand_dims(img, -1)
            img = numpy.ascontiguousarray(img.transpose(2, 0, 1))
            results["img_mini"] = DataContainer(to_tensor(img), stack=True)
        ...
        if results["with_mask"]:
            ...
            if "gt_mask_parts_rle" in results:
                results["gt_mask_parts_rle"] = DataContainer(
                    results["gt_mask_parts_rle"], cpu_only=True, stack=False, pad_dims=None
                )
            if "nums" in results:
                results["nums"] = DataContainer(results["nums"], cpu_only=True, stack=False, pad_dims=None)
            if "omission" in results:
                results["omission"] = DataContainer(results["omission"], cpu_only=True, stack=False, pad_dims=None)
            if "proportion" in results:
                results["proportion"] = DataContainer(results["proportion"], cpu_only=True, stack=False, pad_dims=None)
        return results
