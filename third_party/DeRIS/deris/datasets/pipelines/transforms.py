...
class Resize(object):
    ...
    def _resize_img(self, results):
        if self.keep_ratio:
            img = mmcv.imrescale(
                results["img"], results["scale"], interpolation=self.interpolation, backend=self.backend
            )
    ...
    def _resize_mask(self, results):
        if results["with_mask"]:
            if self.keep_ratio:
                ...
                if "gt_mask_parts" in results:
                    results["gt_mask_parts"] = [part.rescale(results["scale"]) for part in results["gt_mask_parts"]]
            else:
                results["gt_mask"] = results["gt_mask"].resize(results["img_shape"][:2])
                if "gt_mask_parts" in results:
                    results["gt_mask_parts"] = [
                        part.resize(results["img_shape"][:2]) for part in results["gt_mask_parts"]
                    ]
            results["gt_mask_rle"] = maskUtils.encode(numpy.asfortranarray(results["gt_mask"].masks[0]))
            if "gt_mask_parts" in results:
                results["gt_mask_parts_rle"] = [
                    maskUtils.encode(numpy.asfortranarray(part.masks[0])) for part in results["gt_mask_parts"]
                ]
...
class HierResize(object):
    ...
    def __init__(self, img_scale=None, mini_img_scale=None, keep_ratio=True, interpolation="bilinear", backend="cv2"):
        if img_scale is None:
            self.img_scale = None
            self.mini_img_scale = None
        else:
            if isinstance(img_scale, list):
                self.img_scale = img_scale
                self.mini_img_scale = mini_img_scale
            else:
                self.img_scale = [img_scale]
                self.mini_img_scale = [mini_img_scale]
            assert mmcv.is_list_of(self.img_scale, tuple)
            assert mmcv.is_list_of(self.mini_img_scale, tuple)
    ...
    def _random_scale(self, results):
        if len(self.img_scale) == 1:
            scale = self.img_scale[0]
            scale_mini = self.mini_img_scale[0]
        else:
            scale = self._random_select(self.img_scale)
            scale_mini = self._random_select(self.mini_img_scale)

        results["scale"] = scale
        results["scale_mini"] = scale_mini
    ...
    def _resize_img(self, results):
        if self.keep_ratio:
            ...
            img_mini = mmcv.imrescale(
                results["img"], results["scale_mini"], interpolation=self.interpolation, backend=self.backend
            )
            new_h, new_w = img.shape[:2]
            h, w = results["ori_shape"][:2]
            w_scale, h_scale = new_w / w, new_h / h
            ...
            img_mini, _, _ = mmcv.imresize(
                results["img"],
                self.mini_img_scale[0],
                return_scale=True,
                interpolation=self.interpolation,
                backend=self.backend,
            )
...
class Normalize:
    ...
    def __call__(self, results):
        ...
        if "img_mini" in results:
            results["img_mini"] = mmcv.imnormalize(results["img_mini"], self.mean, self.std, self.to_rgb)
...
class LargeScaleJitter:
    ...
    def __init__(
        self, out_max_size=640, jitter_min=0.3, jitter_max=1.4, min_iou_thr=0.3, crop_iou_thr=[0.5, 0.6, 0.7, 0.8, 0.9]
    ):
    ...
    def __call__(self, results):
        ...
        if with_mask:
            ...
            gt_mask_parts = results["gt_mask_parts"]
            gt_mask = gt_mask.rescale(scale, interpolation="bilinear")
            gt_mask_parts = [part.rescale(scale, interpolation="bilinear") for part in gt_mask_parts]
        ...
        if rand_scale > 1.0:
                ...
                if not flag:
                    for iter in range(self.jitter_times):
                        ...
                        if with_bbox:  # rec & res default to rec
                            ...
                            refer_target_index = results["refer_target_index"]
                            iou_parts = [self._mask_overlaps(crop_bbox.astype(numpy.uint32), part) for part in (gt_mask_parts[ind] for ind in refer_target_index)]
                            iou = min(iou, *iou_parts)
            ...
            if with_mask:
                ...
                gt_mask_parts = [gt_mask_part_.crop(crop_bbox) for gt_mask_part_ in gt_mask_parts]

        if with_bbox:
            if isinstance(gt_bbox, list):
                gt_bbox = numpy.array(gt_bbox)
        ...
        if with_mask:
            ...
            results["gt_mask_parts_rle"] = [
                maskUtils.encode(numpy.asfortranarray(part.masks[0])) for part in gt_mask_parts
            ]
