...
from deris.models import VIS_ENCODERS
...
class BEIT3(BEiT3Wrapper):
    ...
    def forward(self, image, question, padding_mask, additional_tokens=None, **kwargs):
        outputs = self.beit3(
        ...
        if additional_tokens is not None:
            nq = additional_tokens.shape[1]
            cls_feat, img_feat, text_feat, addition_feat = (
                x[:, 0],
                x[:, 1 : -(question.shape[-1] + nq)],
                x[:, -(question.shape[-1] + nq) : -nq],
                x[:, -nq:],
            )
            return img_feat, text_feat, cls_feat, addition_feat
        else:
