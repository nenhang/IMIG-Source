...
class DreamOPipeline(FluxPipeline):
    ...
    def load_dreamo_model(self, device, use_turbo=True, version='v1.1', args={}):
        import os
        dreamo_model_dir = args.get("dreamo_model_path", None)
        if dreamo_model_dir is None or not os.path.exists(dreamo_model_dir):
            dreamo_model_dir = "ByteDance/DreamO"
            print(f"Downloading DreamO model from {dreamo_model_dir} ...")
            # download models and load file
            hf_hub_download(repo_id=dreamo_model_dir, filename='dreamo.safetensors', local_dir='models')
            hf_hub_download(repo_id=dreamo_model_dir, filename='dreamo_cfg_distill.safetensors', local_dir='models')
            if version == 'v1':
                hf_hub_download(repo_id=dreamo_model_dir, filename='dreamo_quality_lora_pos.safetensors',
                                local_dir='models')
                hf_hub_download(repo_id=dreamo_model_dir, filename='dreamo_quality_lora_neg.safetensors',
                                local_dir='models')
                quality_lora_pos = load_file(f'{dreamo_model_dir}/dreamo_quality_lora_pos.safetensors')
                quality_lora_neg = load_file(f'{dreamo_model_dir}/dreamo_quality_lora_neg.safetensors')
            elif version == 'v1.1':
                hf_hub_download(repo_id=dreamo_model_dir, filename='v1.1/dreamo_sft_lora.safetensors', local_dir='models')
                hf_hub_download(repo_id=dreamo_model_dir, filename='v1.1/dreamo_dpo_lora.safetensors', local_dir='models')
                sft_lora = load_file(f'{dreamo_model_dir}/v1.1/dreamo_sft_lora.safetensors')
                dpo_lora = load_file(f'{dreamo_model_dir}/v1.1/dreamo_dpo_lora.safetensors')
            else:
                raise ValueError(f'there is no {version}')
        else:
            print(f"Loading DreamO model from {dreamo_model_dir} ...")
            if version == 'v1':
                quality_lora_pos = load_file(f'{dreamo_model_dir}/dreamo_quality_lora_pos.safetensors')
                quality_lora_neg = load_file(f'{dreamo_model_dir}/dreamo_quality_lora_neg.safetensors')
            elif version == 'v1.1':
                ...
                raise ValueError(f'there is no {version}')
        dreamo_lora = load_file(f'{dreamo_model_dir}/dreamo.safetensors')
        cfg_distill_lora = load_file(f'{dreamo_model_dir}/dreamo_cfg_distill.safetensors')
        ...
        if use_turbo:
            if turbo_model_dir := args.get("turbo_model_path", None):
                print(f"Loading turbo lora from {turbo_model_dir} ...")
                self.load_lora_weights(
                    turbo_model_dir, adapter_name='turbo',
                )
            else:
                print(f"Downloading turbo lora from alimama-creative/FLUX.1-Turbo-Alpha ...")
                self.load_lora_weights(
                    hf_hub_download(
